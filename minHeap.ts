export type HeapNode<T extends number | string, U> = {
  value: T;
  key: U;
};

export class MinHeap<T extends number | string, U = undefined> {
  ...
  private bubbleUp(): boolean {
    ...
    let temp: HeapNode<T, U> | undefined = undefined;
    // Keep looping until the parent node is less than the child node
    while (
      parentIndex >= 0 &&
      this._values[parentIndex].value > this._values[index].value
    ) {
   ...
   */
  public extractMin(): T | HeapNode<T, U> {
    ...
    return min.key != undefined ? min : min.value;
  }
